# Validate and preview the production maps and the three serialized palettes.
param([switch]$AttackPreview)
$ErrorActionPreference = 'Stop'
Add-Type -AssemblyName System.Drawing
$root = Split-Path $PSScriptRoot -Parent
$source = Get-Content -Raw (Join-Path $root 'Assets/Scripts/Characters/ArabSoldierZeldaCharacterData.cs')
$directions = @('Front','Back','Left','Right')
$names = @('ArabSoldier-level2','AdvancedArabSoldier-level2','EliteArabSoldier-level2')
$labels = @('Grey / Rank 1','Yellow / Rank 2','Magenta / Rank 3')
$maps = @{}
$symbols = @{}
...
foreach ($direction in $directions) {
    ...
    if (($rows -join '') -notmatch 'C' -or ($rows -join '') -notmatch 'R') { throw "Scarf not visible: $direction" }
    $maps[$direction] = $rows
}
foreach ($row in 0..15) {
    $reversed = $maps.Left[$row].ToCharArray(); [array]::Reverse($reversed)
    if (($reversed -join '') -ne $maps.Right[$row]) { throw 'Side maps are not mirrored' }
}
foreach ($direction in @('Back','Left','Right')) {
    foreach ($row in 9..15) {
        if ($maps[$direction][$row] -match '[CR]') { throw "Scarf extends below upper back: $direction row $row" }
    }
}
foreach ($row in 6..8) {
    if ($maps.Left[$row][9] -match '[CR]' -or $maps.Right[$row][6] -match '[CR]') { throw 'Side back scarf must be only one pixel wide.' }
}
    ...
    if ($pose -eq 3) {
        if ($direction -lt 2) {
            ...
            foreach ($y in 6..8) {
                ...
                $cells[$innerX,$y]=if ($y -eq 6) {'B'} else {'S'}
            }
        } else {
            $idleX = 7
            foreach ($y in 6..8) { foreach ($x in $idleX..($idleX+1)) {
                $cells[$x,$y]=if ($y -eq 6) {'B'} else {'A'}
            } }
        }
        ...
        foreach ($y in $startY..($startY+2)) { foreach ($x in $startX..($startX+3)) {
            $cells[$x,$y] = if ($y -eq ($startY+2)) {'L'} else {'A'}
        } }
...
foreach ($direction in $directions) {
    ...
    foreach ($x in $columns) {
        if ($maps[$direction][7][$x] -ne 'L') { throw "Missing visible cuff: $direction" }
...
$bmp = [Drawing.Bitmap]::new(800,690)
$g = [Drawing.Graphics]::FromImage($bmp)
$g.Clear([Drawing.Color]::FromArgb(25,32,43))
$font = [Drawing.Font]::new('Arial',11)
$baselinePalette = @{}
$scriptGuid = [regex]::Match((Get-Content -Raw (Join-Path $root 'Assets/Scripts/Characters/ArabSoldierZeldaCharacterData.cs.meta')), 'guid: (\w+)').Groups[1].Value
foreach ($tier in 0..2) {
    $prefab = Get-Content -Raw (Join-Path $root "Assets/Prefabs/Level2NPCS/$($names[$tier]).prefab")
    if (-not $prefab.Contains("guid: $scriptGuid")) { throw 'Missing soldier data script reference' }
    ...
    foreach ($symbol in $symbols.Keys) {
        ...
        if ($tier -eq 0) {$baselinePalette[$field]=$m.Value}
        elseif ($symbol -notin @('C','R') -and $baselinePalette[$field] -ne $m.Value) { throw 'Only the scarf should vary by rank' }
    }
    foreach ($d in 0..3) {
...
$previewFile = if ($AttackPreview) {'Docs/ArabSoldiers-attack-preview.png'} else {'Docs/ArabSoldiers-preview.png'}
$output=Join-Path $root $previewFile
$bmp.Save($output,[Drawing.Imaging.ImageFormat]::Png)
$font.Dispose(); $g.Dispose(); $bmp.Dispose()
Write-Output "PASS: 16 connected idle/walk/attack poses, mirrored sides, three scarf-only palettes, ranks and prefab references. Preview: $output"
