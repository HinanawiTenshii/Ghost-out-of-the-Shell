# Validate and preview the Alchemist production maps and serialized palette.
param([switch]$AttackPreview)
$ErrorActionPreference = 'Stop'
Add-Type -AssemblyName System.Drawing
$root = Split-Path $PSScriptRoot -Parent
$source = Get-Content -Raw (Join-Path $root 'Assets/Scripts/Characters/AlchemistZeldaCharacterData.cs')
$directions = @('Front','Back','Left','Right')
$names = @('Alchemist-level2')
$labels = @('Alchemist')
$maps = @{}
$symbols = @{}
...
# Mirror the body and head; only hip accessories differ (bottle vs leather pouch).
foreach ($row in 0..9) {
...
if (($maps.Front -join '') -notmatch 'Q' -or ($maps.Front -join '') -notmatch 'P') { throw 'Missing glass/potion detail' }
    ...
    if ($pose -eq 3) {
        if ($direction -lt 2) {
            ...
            foreach ($y in 6..8) { foreach ($x in $idleX..($idleX+1)) {
                $cells[$x,$y]=if ($y -eq 6) {'B'} elseif (($direction -eq 2 -and $x -eq 8) -or ($direction -eq 3 -and $x -eq 7)) {'T'} else {'A'}
            } }
        }
...
$scriptGuid = [regex]::Match((Get-Content -Raw (Join-Path $root 'Assets/Scripts/Characters/AlchemistZeldaCharacterData.cs.meta')), 'guid: (\w+)').Groups[1].Value
foreach ($tier in 0..0) {
...
$previewFile = if ($AttackPreview) {'Docs/Alchemist-attack-preview.png'} else {'Docs/Alchemist-preview.png'}
$output=Join-Path $root $previewFile
$bmp.Save($output,[Drawing.Imaging.ImageFormat]::Png)
$font.Dispose(); $g.Dispose(); $bmp.Dispose()
Write-Output "PASS: 16 connected idle/walk/attack poses, no duplicate attacking hands, matching side body, potion detail and prefab references. Preview: $output"
