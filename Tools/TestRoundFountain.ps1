...
$path=Join-Path $root 'Assets/Prefabs/Decorations/Fountain-Round.prefab'
...
if(!(Get-Content -Raw ($path+'.meta')).Contains('guid: 605f8d1273fa48da8514ce9f973a1de9')){throw 'Fountain GUID changed'}
$rt=$blocks|Where-Object{$_ -match '^--- !u!4 &6553058051394660024\b'}
if(!$rt.Contains('m_LocalScale: {x: 8, y: 8, z: 8}') -or !$rt.Contains('m_LocalPosition: {x: 0, y: 0, z: 0}')){throw 'Existing placement/scale changed'}
$circleCollider=$blocks|Where-Object{$_ -match '^--- !u!58 &6553058051394660026\b'}
if(!$circleCollider -or !$circleCollider.Contains('m_Radius: 0.5') -or !$circleCollider.Contains('m_IsTrigger: 0') -or !$circleCollider.Contains('m_Enabled: 1')){throw 'Solid circular collider mismatch'}
if($prefab -match 'BoxCollider2D:'){throw 'Square collider remains'}
$circleGuid='a86470a33a6bf42c4b3595704624658b'
$circleMeta=Get-Content -Raw "$root/Library/PackageCache/com.unity.2d.sprite@1.0.0/Editor/ObjectMenuCreation/DefaultAssets/Textures/v2/Circle.png.meta"
if(!$circleMeta.Contains("guid: $circleGuid") -or !$circleMeta.Contains('internalID: -2413806693520163455') -or !$circleMeta.Contains('spritePixelsToUnits: 256')){throw 'Unity Circle reference invalid'}
...
foreach($b in $blocks){
    ...
    if($type -eq '4' -and !$b.Contains('stripped')){
        ...
        if([double]$pos.Groups[3].Value -ne 0 -or $b -notmatch 'm_LocalRotation: \{x: -?0, y: -?0, z: -?0, w: 1\}'){throw 'Fountain must remain flat and top-down'}
    ...
    if($type -eq '212'){
        if(!$b.Contains("guid: $circleGuid") -and !$b.Contains('guid: 311925a002f4447b3a28927169b83ea6')){throw 'Non-primitive geometry'}
        $c=[regex]::Match($b,'m_Color: \{r: ([^,]+), g: ([^,]+), b: ([^,]+), a: ([^}]+)')
        $renderers+=@{go=$go;circle=$b.Contains("guid: $circleGuid");order=[int][regex]::Match($b,'m_SortingOrder: (-?\d+)').Groups[1].Value;color=@([float]$c.Groups[1].Value,[float]$c.Groups[2].Value,[float]$c.Groups[3].Value,[float]$c.Groups[4].Value)}
    }
}
if($renderers.Count -ne 11 -or @($renderers|Where-Object circle).Count -ne 7){throw 'Expected 7 Circle and 4 Square sprites'}
$original=Get-Content -Raw "$root/Assets/Prefabs/Decorations/喷泉.prefab"
if([regex]::Matches($original,'guid: 311925a002f4447b3a28927169b83ea6').Count -ne 11 -or !$original.Contains('BoxCollider2D:')){throw 'Original square fountain changed'}
$originalBlocks=[regex]::Split($original,'(?m)(?=^--- !u!)')
foreach($r in $renderers){
    $old=$originalBlocks|Where-Object{$_ -match '^--- !u!212 ' -and $_.Contains("m_GameObject: {fileID: $($r.go)}")}
    $new=$blocks|Where-Object{$_ -match '^--- !u!212 ' -and $_.Contains("m_GameObject: {fileID: $($r.go)}")}
    if([regex]::Match($old,'m_Color: [^\r\n]+').Value -ne [regex]::Match($new,'m_Color: [^\r\n]+').Value){throw 'Palette diverges from source'}
}
...
$originalEmitter=$originalBlocks|Where-Object{$_ -match '^--- !u!1001 &6553058052248252519\b'}
if($emitter.Trim() -ne $originalEmitter.Trim()){throw 'Original spray overrides not preserved'}
...
foreach($r in ($renderers|Sort-Object order)){
    ...
    $rect=[Drawing.RectangleF]::new([float](370+($t.pos[0]-$t.size[0]/2)*640),[float](370-($t.pos[1]+$t.size[1]/2)*640),[float]($t.size[0]*640),[float]($t.size[1]*640))
    if($r.circle){
        if($t.size[0] -ne $t.size[1] -or $t.pos[0] -ne 0 -or $t.pos[1] -ne 0){throw 'Circle layers must be concentric'}
        $g.FillEllipse($brush,$rect)
    }else{
        $cornerX=[Math]::Abs($t.pos[0])+$t.size[0]/2;$cornerY=[Math]::Abs($t.pos[1])+$t.size[1]/2
        if([Math]::Sqrt($cornerX*$cornerX+$cornerY*$cornerY) -gt .439){throw 'Glint outside circular water'}
        $g.FillRectangle($brush,$rect)
    }
    $brush.Dispose()
}
$preview=Join-Path $root 'Docs/Fountain-Round-preview.png'
...
Write-Output 'PASS: independent centered round prefab; 7 Circle + 4 Square sprites; source palette and spray unchanged; fitted solid CircleCollider2D; details and particle reach stay within basin.'
