...
/// <summary>Level 2 artisan with a hooded robe, brass goggles and decorative alchemy bottles.</summary>
public sealed class AlchemistZeldaCharacterData : ZeldaCharacterData
{
    ...
    [SerializeField] private Color attackVisualTint = new Color(0.72f, 0.76f, 0.8f, 0.82f);
    [Header("Alchemist Appearance / 炼金术士外观")]
    [SerializeField] private Color robeColor = new Color(0.18f, 0.38f, 0.34f, 1f);
    [SerializeField] private Color robeShadow = new Color(0.10f, 0.24f, 0.23f, 1f);
    [SerializeField] private Color robeHighlight = new Color(0.33f, 0.52f, 0.43f, 1f);
    [SerializeField] private Color trimColor = new Color(0.70f, 0.56f, 0.29f, 1f);
    [SerializeField] private Color apronColor = new Color(0.54f, 0.49f, 0.36f, 1f);
    [SerializeField] private Color faceColor = new Color(0.76f, 0.52f, 0.34f, 1f);
    [SerializeField] private Color eyeColor = new Color(0.04f, 0.10f, 0.12f, 1f);
    [SerializeField] private Color beltColor = new Color(0.32f, 0.22f, 0.13f, 1f);
    [SerializeField] private Color bootsColor = new Color(0.20f, 0.13f, 0.09f, 1f);
    [SerializeField] private Color potionColor = new Color(0.64f, 0.34f, 0.73f, 1f);
    [SerializeField] private Color glassColor = new Color(0.69f, 0.81f, 0.76f, 1f);
    [SerializeField, Min(1f)] private float walkFramesPerSecond = 7f;

    // 16x16, top-to-bottom: A/S/L=robe, G=brass trim, T=apron,
    // F/E=skin/eyes, B=leather, K=boots, P/Q=potion/glass.
    ...
    private Sprite CreateSprite(int direction, int pose)
    {
        ...
        if (pose == 3)
        {
            ...
            for (int x = startX; x < startX + 4; x++)
                pixels[y * 16 + x] = y == startY + 2 ? robeHighlight : robeColor;
    ...
    private void RestoreBodyBehindAttackingArm(Color[] pixels, int direction)
    {
        if (direction < 2)
        {
            ...
            for (int y = 6; y <= 8; y++)
            {
                ...
                pixels[y * 16 + innerX] = y == 6 ? beltColor : robeShadow;
            }
        }
        else
        {
            ...
            for (int x = startX; x < startX + 2; x++)
                pixels[y * 16 + x] = y == 6 ? beltColor :
                    ((direction == 2 ? x == 8 : x == 7) ? apronColor : robeColor);
        }
    }
